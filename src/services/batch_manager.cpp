...
bool BatchManager::add_task(const std::string& batch_id, const models::AddTaskRequest& req) {
    ...
    {
        std::lock_guard<std::mutex> lock(db_mutex_);
        std::string check_query = "SELECT status, options FROM batches WHERE id = '" + escaped_batch_id + "'";
    ...
    {
        std::lock_guard<std::mutex> lock(db_mutex_);
        std::string count_query = "SELECT COUNT(*) FROM tasks WHERE batch_id = '" + escaped_batch_id + "'";
...
bool BatchManager::start_batch(const std::string& batch_id) {
    ...
    {
        std::lock_guard<std::mutex> lock(db_mutex_);
        std::string check_query = "SELECT status FROM batches WHERE id = '" + escaped_batch_id + "'";
        if (mysql_query(conn, check_query.c_str())) return false;
        ...
        std::string update_query = "UPDATE batches SET status = 'awaiting' WHERE id = '" + escaped_batch_id + "'";
        ...
        std::thread([this]() {
            while (true) {
                ...
                for (const auto& b : awaiting_batches) {
                    ...
                    long long wait_duration_ms = b.wait_ms;
                    ...
                    if (!tasks_to_do.empty()) {
                        std::thread([this, tasks_to_do, b_id = b.id]() {
                            ...
                            for (size_t i = 0; i < tasks_to_do.size(); ++i) {
                                ...
                                {
                                    std::lock_guard<std::mutex> lock(db_mutex_);
                                    local_url = success ? "file://" + escape_string(t_conn, t.dest) : "";
                                    mysql_query(t_conn, ("UPDATE tasks SET status = '" + final_status + "', local_url = '" + local_url + "' WHERE id = '" + t.id + "'").c_str());
                                }
                                            ...
                                            if (res_ptr) {
                                                ...
                                                if (res_row) {
                                                    try {
                                                        ...
                                                        res_val = opts.value("waveform_resolution", 512);
                                                    } catch (...) {}
                                                }
...
std::optional<models::Batch> BatchManager::get_batch(const std::string& batch_id) {
    ...
    {
        std::lock_guard<std::mutex> lock(db_mutex_);
        std::string query = "SELECT status, options FROM batches WHERE id = '" + escaped_batch_id + "'";
        ...
        try {
            if (row[1]) {
                json opts = json::parse(row[1]);
                b.options.wait_duration = opts.value("wait_duration", "5s");
                b.options.max_retries = opts.value("max_retries", 0);
                b.options.max_batch_size = opts.value("max_batch_size", 0);
                b.options.max_batch_storage = opts.value("max_batch_storage", "");
                b.options.allowed_services = opts.value("allowed_services", std::vector<std::string>{});
                b.options.delete_after = opts.value("delete_after", "");
                b.options.waveform_resolution = opts.value("waveform_resolution", 512);
            }
        } catch (...) {}
        ...
        std::string task_query = "SELECT t.id, t.file_id, t.destination_path, t.status, t.local_url, "
                                 ...
                                 "WHERE t.batch_id = '" + escaped_batch_id + "'";
...
std::optional<models::Task> BatchManager::get_ingested_task(const std::string& task_id) {
    MYSQL* conn = DatabaseService::get_instance().get_connection();
    std::string escaped_task_id = escape_string(conn, task_id);
